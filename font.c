 ...
 * font.c
 * 
 * A simple opengl font library that uses GNU freetype2
 * 
...
//defined as Font in the interface
 ...
 **********************************************************************/
 ...
 static 
 void 
 Font_MakeGlyphTexture(FT_Bitmap * bitmap, GLuint textureID, GLuint listID)
 {
	int w = bitmap->width;
	int h = bitmap->rows;
 ...
 * Font_CreateGlyph
 * 
 * creates the glyph from the given font face and character
 **********************************************************************/
 
 static 
 FT_Glyph 
 Font_CreateGlyph(FT_Face face, unsigned char ch)
 {
	FT_Glyph glyph;
...
static 
void 
Font_MakeDisplayList(FT_Face face, unsigned char ch, GLuint list_base, GLuint * tex_base)
{
	GLuint textureID = tex_base[(int)ch];
	GLuint listID = list_base + (GLuint)ch;
	float x, y;

	FT_Glyph glyph = Font_CreateGlyph(face, ch);
	...
	Font_MakeGlyphTexture(bitmap, textureID, listID);
	...
	glNewList(listID, GL_COMPILE);
		...
		//should also be constant for monospace font
 ...
 * Font_Init
 * 
 * creates a font from the specified font name and height
 **********************************************************************/

Font* 
Font_Init(const char * fname, unsigned int height) 
{
	FT_Library library;
	FT_Face face;
	unsigned char ch;
	...
	for(ch = 0; ch < 255; ++ch) {
		Font_MakeDisplayList(face, ch, fnt->list_base, fnt->textures);
	}

	//display lists are created; don't need the font face or library
	FT_Done_Face(face);
 ...
 * Font_Destroy
 * 
 * destroys and cleans up the specified font
 **********************************************************************/
 
void 
Font_Destroy(Font * fnt) 
{
	//list_base
 ...
 * Font_Print
 * 
 * the print function in all its glory; prints using the font
 **********************************************************************/

void
Font_Print(Font * fnt, Frame * frm, int y)
{
	static int x = 320;
	GLuint font = fnt->list_base;
	//make the line height bigger than the font so space between lines
